...
<script>
    ...
    // Function to create continuous blinking radius wave around markers
    function createContinuousBlinkingRadiusWave(latlng) {
        // Create a circle overlay with initial radius
        let circle = L.circle(latlng, {
            radius: 0, // Start with zero radius
            color: 'green', // Adjust the color of the circle
            opacity: 0.2, // Adjust the opacity of the circle
        }).addTo(map);

        // Animate the radius continuously to create the blinking radius wave
        let radius = 0;
        setInterval(function() {
            circle.setRadius(radius);
            radius += 50; // Adjust the speed of the wave by changing this value
            if (radius > 500) { // Reset radius when it reaches maximum
                radius = 0;
            }
        }, 100); // Adjust the interval of the animation
    }
        ...
        data.forEach(function(markerData) {
            ...
            if (!isNaN(lat) && !isNaN(long)) {
                ...
                let popupContent = '<div class="container">' +
                    '<div class="card ">' +
                    '<div class="header text-bg-primary mb-3 text-center"><p>' + markerData.lokasi + '</p></div>' +
                    '<div class="card-body">' +
                    ...
                    '</tr>' +
                    '<tr>' +
                    '<th scope="row">PREASURE</th>' +
                    '<th scope="row">000.0 Bar</th>' +
                    '</tr>' +
                    '<tr>' +
                    '<th scope="row">TURBIDITY</th>' +
                    '<th scope="row">000.0 NTu</th>' +
                    '</tr>' +
                    '<tr>' +
                    '<th scope="row">PH</th>' +
                    '<th scope="row">000.00 pH</th>' +
                    '</tr>' +
                    '<tr>' +
                    '<th scope="row">RESIDU CHLOR</th>' +
                    '<th scope="row">000.00 Cl</th>' +
                    '</tr>' +
                    '</tbody>' +
                    '</table>' +
                    '</div>' +
                ...
                // Create continuous blinking radius wave around the marker
                createContinuousBlinkingRadiusWave([lat, long]);
