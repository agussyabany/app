...
class KibFController extends Controller
{
    ...
    {
        $konstruksi = KibF::join('barangs','kib_f_s.id_barang','barangs.id')
                        ->join('divisis','kib_f_s.id_div','divisis.id')
                        ->where('kib_f_s.id_dep',$dep)
                        ->where('id_lokasi',$lok)
                        ->where('id_div',$div)
                        ...
                        $nama = Auth::user()->name;
                        $nip = Auth::user()->nip;
                        return view('admin.pages.aset.print.printKonstruksi',compact(['konstruksi','i','lokasi','departemen','divisi','nama','tglIndo','struktur','nip']));
    }
}
